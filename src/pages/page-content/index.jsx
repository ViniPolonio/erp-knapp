...
class PageContent extends Component {
      ...
      <li key={`${data.to}-${data.title}`}>
        <Link
          className={classnames(
            "nav-link",
            activeScrollSpy && activeScrollSpy === data.to ? "active" : ""
          )}
          to={data.to}
        ...
        {data.sub ? (
          <ul className="nav flex-column">
            {data.sub.map((innerData) => this.getNavigationItem(innerData))}
          </ul>
        ) : (
          ""
        )}
  ...
  render() {
    ...
    if (sideNav && sideNav.length) {
      ...
      content = (
        <Row className="align-items-start mr-0">
          ...
          <Col
            xl={2}
            className="d-none d-xl-block rui-scrollbar rui-page-sidebar-sticky"
          >
    ...
    return <div className="rui-page-content">{content}</div>;
  }
}
